...
- **音频特征提取器**：使用`ClapCap`（Elizalde 等，2023b）作为音频特征提取器的主干。`ClapCap`被硬编码为以7秒的44.1kHz原始音频作为输入，然后将音频转换为跳长为320、窗口长度为1024、64维的Mel频谱图，最后输出**一个**1024维度的向量表示。每7秒的片段视为一个窗口，并使用滑动窗口从较长的音频中提取特征，连续窗口之间的重叠为`7×0.75=5.25`秒。这种设计的直觉是为了捕捉在单个融合表示向量中可能被忽略的长距离和时间信息。使用最多 16 个滑动窗口，因此支持最多33.25秒的音频长度。  
*批量训练时不足的滑动窗口将被填充，超出的滑动窗口将被裁剪，每个音频生成16个token*
...
作者从 `MiraData` 和 `Video Recap` 中选择了 10 万个多样化的视频，将这些视频分割成 10 秒的片段，并使用 `Qwen2-VL-2B-Instruct` 生成视频字幕，使用 `Qwen2-Audio` 生成音频字幕，然后进行相似度对比。为确保多样性和减少冗余，过滤掉音频-视觉相似度超过阈值的片段 。然后，提示 GPT-4o（2024-05-13）生成以音频为中心的字幕，排除视觉属性并强调声音事件。
...
除了V1用到的数据集之外，作者还提出了两个新的数据集：

##### AudioSkills: An Expert Audio Reasoning Dataset

大多数现有的音频数据集都侧重于表面级属性，例如声学事件或类别分类。

因此作者引入了 `AudioSkills`，一个高质量的、针对特定技能的合成数据集，旨在优先发展推理和解决问题的能力。具体来说，作者结合了开源声音和音乐数据集、合成音频和 `GPT-4o` 提示，以及任何可用的元数据来创建问答对。`AudioSkills` 专注于长度小于30秒的音频，以实现开源数据集的有效扩展。该数据集涵盖了七个不同的技能，这些技能因其可能性、相关性和消融洞察而被选中，如下所示：

* Temporal Reasoning：时间推理
* Attribute Identification：属性识别
* Counting：计数
* Contextual Sound Event Reasoning：上下文声音事件推理
* Contextual Speech Event Reasoning：上下文语音事件推理
* Information Extraction：信息提取
* General Reasoning：一般推理，包括不属于上述特定技能类型的题目

共计 4.2M 个 QA 对 *这么多！！！*

![V2Audioskill](../images/Audio-Flamingo/V2Audioskill.png)

##### LongAudio: A Long Audio Understanding Dataset

`LongAudio` 是第一个大规模长音频理解数据集，包含超过 80K 个独特的音频和大约 263K 个 AQA 对。音频来源于公开的长视频数据集，包括 `MiraData` 的一个子集（包含从自然场景到游戏的多样化内容）和整个 `Video ReCap`（日常活动的自中心视频）。为确保多样性，作者通过聚类视频并从每个簇中仔细选择样本来过滤 `MiraData` 。
...
![V2longaudio信息](../images/Audio-Flamingo/V2longaudio信息.png)

分别使用 `Qwen2-VL-2B-Instruct` 和 `Qwen2-Audio` 为视频和音频片段生成字幕。然后使用 `GPT-4o` 生成问题，这些问题要求提供字幕、挑战模型进行推理或从长音频中提取信息。

![V2longaudio流程](../images/Audio-Flamingo/V2longaudio流程.png)

##### LongAudioBench

长音频问答评估基准数据集，作者从 `LongAudio` 中抽取约 10% 的样本，按 QA 类型比例分配，进行专家人工验证。。随后，三位作者对纠正后的样本进行质量检查。`LongAudioBench` 的最终版本有 2,429 个实例。

#### 训练策略

作者使用 3 阶段课程学习策略训练 AF2，逐步增加音频上下文长度并提高数据质量：

* **预训练**：此阶段侧重于多模态对齐，将音频表示与LLM对齐。训练中利用大规模且可能存在噪声的分类和字幕数据集，包含少量高质量问答数据以改善预训练。在此阶段，CLAP 和 LLM 层被冻结，只有音频表示转换层和门控交叉注意力层可训练。音频上下文限制在 w=3 窗口（30 秒）
* **指令微调**：此阶段侧重于通过学习技能提高音频理解和推理。训练中使用高质量短音频分类、字幕和问答数据集。在此阶段，只有 LLM 层被冻结，CLAP 模型、音频表示转换层和门控交叉注意力层可训练。音频上下文长度增加到 w=9 窗口（1.5 分钟） 
* **长音频微调**。此阶段侧重于上下文长度扩展和训练长音频推理的能力。训练中采用 `LongAudio` 数据集，仅音频表示转换层和门控交叉注意力层可训练，并增加至 w=30 窗口（5 分钟）
